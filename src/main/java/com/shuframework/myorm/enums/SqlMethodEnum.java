...
public enum SqlMethodEnum {
	...
	INSERT_ONE_ALL_COLUMN("insertAllColumn", "插入一条数据（全部字段插入）", "INSERT INTO %s (%s) VALUES (%s)"),
	
    /**
     * 修改
     */
    UPDATE_BY_ID("updateById", "根据ID 选择修改数据", "UPDATE %s set %s WHERE %s=#{%s} %s"),
...
//    UPDATE("update", "根据 whereEntity 条件，更新记录", "UPDATE %s %s %s"),
     ...
     * 逻辑删除
     */
    LOGIC_DELETE_BY_ID("deleteById", "根据ID 逻辑删除一条数据", "UPDATE %s set %s WHERE %s=#{%s}"),
...
//    LOGIC_DELETE_BATCH_BY_IDS("deleteBatchIds", "根据ID集合，批量逻辑删除数据", "UPDATE %s %s WHERE %s IN (%s)"),
     ...
     * 查询
     */
    SELECT_BY_ID("selectById", "根据ID 查询一条数据", "SELECT %s FROM %s WHERE id=?"),
